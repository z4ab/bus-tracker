...
async def test_cache_miss_returns_none(db_path: str) -> None:
    loaded = await load_cached_static(
        "https://example.com/unknown.zip", db_path=db_path
    )
...
async def test_upsert_replaces_existing_entry(db_path: str) -> None:
    ...
    await save_cached_static(
        feed_url, routes_v1, stops_v1, last_modified="old", db_path=db_path
    )

    routes_v2 = {"2": {"route_id": "2"}}
    await save_cached_static(
        feed_url, routes_v2, stops_v1, last_modified="new", db_path=db_path
    )
...
async def test_clear_single_url(db_path: str) -> None:
    ...
    assert (
        await load_cached_static("https://example.com/a.zip", db_path=db_path) is None
    )
    assert (
        await load_cached_static("https://example.com/b.zip", db_path=db_path)
        is not None
    )
...
async def test_clear_all(db_path: str) -> None:
    ...
    assert (
        await load_cached_static("https://example.com/b.zip", db_path=db_path) is None
    )
